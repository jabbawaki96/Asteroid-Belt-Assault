...
namespace Asteroid_Belt_Assault
{
    public static class SoundManager
    {
        ...
        public static void Initialize(ContentManager content)
        {
            try
            {
                playerShot = content.Load<SoundEffect>(@"SFX\Shot1");
                enemyShot = content.Load<SoundEffect>(@"SFX\Shot2");

                for (int x = 1; x <= explosionCount; x++)
                {
                    explosions.Add(content.Load<SoundEffect>(@"SFX\Explosion" + x.ToString()));
                }
            }
            catch
            {
