...
namespace Asteroid_Belt_Assault
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        enum GameStates { TitleScreen, Playing, PlayerDead, GameOver, Pause };
        ...
        protected override void LoadContent()
        {
            ...
            starField = new StarField(this.Window.ClientBounds.Width, this.Window.ClientBounds.Height, 200, new Vector2(0, 30f), spriteSheet, new Rectangle(0, 450, 2, 2)); // The Star Field
            explosionManager = new ExplosionManager(spriteSheet, new Rectangle(0, 100, 50, 50), 3, new Rectangle(0, 450, 2, 2));    // Explosion
           
            asteroidManager = new AsteriodManager(10, spriteSheet, new Rectangle(0, 0, 50, 50), 20, this.Window.ClientBounds.Width, this.Window.ClientBounds.Height);   //Asteroids
            playerManager = new PlayerManager(spriteSheet, new Rectangle(0, 150, 50, 50), 3, new Rectangle(0, 0, this.Window.ClientBounds.Width,// Player
                this.Window.ClientBounds.Height));
            enemyManager = new EnemyManager(spriteSheet, new Rectangle(0, 200, 50, 50), 6, playerManager, new Rectangle(0, 0, this.Window.ClientBounds.Width,   // Enemy
                this.Window.ClientBounds.Height));
            collisionManager = new CollisionManager(asteroidManager, playerManager, enemyManager, explosionManager);    // Collision
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            // checks for events, such as the player dying, and player input so that it can continue to run the game in the correct state.
            switch (gameState)
            {
                ...
                case GameStates.Playing:
                    ...
                    if (GamePad.GetState(PlayerIndex.One).Buttons.LeftShoulder == ButtonState.Pressed || Keyboard.GetState(PlayerIndex.One).IsKeyDown(Keys.P))
                    {
                        pauseScreenTimer = 1.0f;
                ...
                case GameStates.PlayerDead:
                    ...
                    if ((GamePad.GetState(PlayerIndex.One).Buttons.Y == ButtonState.Pressed) || (playerDeathTimer >= playerDeathDelayTime) || (Keyboard.GetState(PlayerIndex.One).IsKeyDown(Keys.R)))
                    {
                        resetGame();
                        gameState = GameStates.Playing;
                    }
                ...
                case GameStates.Pause:
                    pauseScreenTimer -= (float)gameTime.ElapsedGameTime.TotalSeconds;
                    if (pauseScreenTimer <= 0.0f)
                    {
                        if((GamePad.GetState(PlayerIndex.One).Buttons.RightShoulder == ButtonState.Pressed) || (Keyboard.GetState(PlayerIndex.One).IsKeyDown(Keys.O)))
                        {
                            gameState = GameStates.Playing;
                        }
                    }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {
                spriteBatch.DrawString(pericles14, "Press (Y) or r to RESPAWN", new Vector2(this.Window.ClientBounds.Width / 2 - pericles14.MeasureString("G A M E  O V E R !").X / 2, 50), Color.White);
            }
            //
